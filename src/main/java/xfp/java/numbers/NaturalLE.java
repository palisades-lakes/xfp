...
public final class NaturalLE implements Natural {
  ...
  private final long addByWords (final long u,
                                 final int iShift,
                                 final int nt,
                                 final int[] vv) {
    long sum = loWord(u);
  ...
  private final long addByBits (final long u,
                                final int iShift,
                                final int bShift,
                                final int nt,
                                final int[] vv) {
  ...
  public final NaturalLE add (final long u,
    ...
    int i;
    if (0==bShift) { 
      sum = addByWords(u,iShift,nt,vv); 
      i=iShift+2; }
    else { 
      sum = addByBits(u,iShift,bShift,nt,vv); 
      i=iShift+3; }
  ...
  private final void addByWords (final int nt,
    ...
    int i=2;
    for (;i<nt+iShift;i++) {
      if (i>=iShift) { sum += uword(i-iShift); }
      vv[i] = (int) sum;
      sum = (sum>>>32); } 
    vv[i] = (int) sum; }

  private final void addByBits (final int nt,
                                final int iShift,
                                final int bShift,
                                final long u,
                                final int[] vv) {
  ...
  public final NaturalLE add (final int upShift,
    ...
    final int[] vv = new int[n+1];
    if (0==bShift) { addByWords(nt,iShift,u,vv); }
    else { addByBits(nt,iShift,bShift,u,vv); }
    return unsafe(vv); }
  ...
  private final void subtractByWords (final int nt,
                                      final int iShift,
                                      final long u,
                                      final int[] vv) {
    long dif = -loWord(u);
  ...
  private final void subtractByBits (final int nt,
                                     final int iShift,
                                     final int bShift,
                                     final long u,
                                     final int[] vv) {
  ...
  public final NaturalLE subtract (final int upShift,
    ...
    if (0==bShift) { subtractByWords(nt,iShift,u,vv); }
    else { subtractByBits(nt,iShift,bShift,u,vv); }
    return unsafe(vv); }
  ...
  public final NaturalLE shiftDown (final int downShift) {
    //assert isValid();
    //assert 0<=shift;
    if (downShift==0) { return this; }
    final int iShift = (downShift>>>5);
    final int n0 = hiInt();
    final int n1 = n0-iShift;
    // shifting all bit off the end, covers zero input case
    if (0>=n1) { return zero(); }
  ...
  public static final NaturalLE valueOf (final long u) {
    ...
    final int lo = (int) u;
    final int hi = (int) (u>>>32);
    if (0==hi) { return unsafe(new int[] {lo},1); }
